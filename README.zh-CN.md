...
- `create_diagram` 会先校验并保存版本，再打开 Glimpse 预览。启用交互预览时，工具会等待审阅结束：`确认并关闭` 接受当前版本并关闭面板，`请求修改` 只展开反馈输入，`关闭` 或取消不代表确认。工具结果包含审阅状态、版本和完整反馈原文。
...
- `/xpi-diagram` 负责预览配置，并可重开最近保存的图表，但命令本身不等待审阅。没有待返回工具时，反馈由宿主在空闲时投递、忙碌时 steer；关闭预览、无界面或 Glimpse 不可用时直接返回已保存结果。
## 技术栈
