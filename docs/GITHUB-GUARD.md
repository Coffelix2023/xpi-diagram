...
本文件只描述 `xpi-diagram` 的仓库级 GitHub 约束。
...
- `xpi-diagram` 目前按**阶段一**处理：单人快速迭代优先。
- **本项目不设工作分支**：始终在 `main` 上直接提交与发布（用户 2026-09-09 明确约定，覆盖默认的"先分支后 PR"规则）。
- 当前仓库不启用 `release-please`。
- 无远端、无历史提交的新脚手架允许在当前主分支创建首次本地提交；这是直推保护分支规则之外的初始化例外。
- 工作分支（如 `fix/diagram-review-layout`）合并后可删除，日常不再创建。
